...
main:
	...
	add $t5, $zero, $zero					# use two registers as flag
	add $t6, $zero, $zero					# second is for not printing a space first
	add $t7, $zero, $zero					# third is for seperate numbers and strings
...
read_char:									# the main function of the program
	...
	bne $t4, $zero, A_Z
	add $t5, $zero, $zero
	bne $t7, $zero, store_byte				# check the flag for a number 
	addi $t7, $zero, 1						# after the string, change the flag
	j add_space								# and add a 'space' before the number
	...
	add $t5, $zero, $zero
	addi $t6, $zero, 1
	beq $t7, $zero, store_byte				# check the flag for a number before
	addi $t7, $zero, 0						# the string, change the flag
	j add_space								# and add a 'space' after the number
	...
	add $t5, $zero, $zero
	addi $t6, $zero, 1
	beq $t7, $zero, store_byte				# it's the same code for checking
	addi $t7, $zero, 0						# for a number like the label A_Z
	j add_space
	
	add_space:
	
	beq $t6, $zero, store_byte				# a new label for adding a 'space'
	li $t4, 32								# but not if space is the first 'space'
	sb $t4, 0($s0)							# of the output string
	addi $s0, $s0, 1
	j store_byte
